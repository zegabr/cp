...
struct Graph {
    ...
    void add_edge(int v, int w, int u) {
...
bool bfs(Graph &g, int s, int t) {
    ...
    while (!queue.empty()) {
        ...
        queue.pop();
        for (int i: g.adj[v]) {
            int w = g.edges[i].v, u = g.edges[i].u;
            if (level[w] == -1 && u > 0) {
                level[w] = level[v] + 1;
                queue.push(w);
            }
...
vector<int> ptr;
int augment(Graph &g, int v, int t, int cap) {
    if (v == t) return cap;
    for (int &p = ptr[v]; p < (int)g.adj[v].size(); p++) {
        int i = g.adj[v][p];
        int w = g.edges[i].v, u = g.edges[i].u;
        if (level[w] == level[v] + 1 && u > 0) {
            int eps = augment(g, w, t, min(cap, u));
            if (eps > 0) {
                g.edges[i].u -= eps;
                g.edges[i ^ 1].u += eps;
                return eps;
