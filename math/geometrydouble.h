...
namespace Geometry{
  ...
  bool ptInSegment(PT a, PT b, PT c) {
    bool x = std::min(a.x, b.x) <= c.x && c.x <= std::max(a.x, b.x);
    bool y = std::min(a.y, b.y) <= c.y && c.y <= std::max(a.y, b.y);
  ...
  bool ptInsideTriangle(PT p, PT a, PT b, PT c) {
    if(cross(b-a, c-b) < 0) std::swap(a, b);
  ...
  bool pointInConvexPolygon( std::vector<PT> &p, PT q) {
    PT pivot = p[0];
    int x = 1, y = p.size();
    while(y-x != 1) {
      int z = (x+y)/2;
  ...
  bool pointInPolygon( std::vector<PT> &p, PT q) {
    bool c = 0;
  ...
  bool pointOnPolygon( std::vector<PT> &p, PT q) {
    for(int i = 0; i < p.size(); i++)
  ...
  std::vector<PT> circleLineIntersection(PT a, PT b, PT c, double r) {
    std::vector<PT> ans;
    b = b - a;
    a = a - c;
    double x = dot(b, b);
  ...
  std::vector<PT> circleCircleIntersection(PT a, PT b, double r, double R) {
    std::vector<PT> ans;
    double d = sqrt(dist2(a, b));
    if (d > r + R || d + std::min(r, R) < std::max(r, R)) return ans;
  ...
  double ComputeSignedArea( std::vector<PT> &p) {
    double area = 0;
    for(int i = 0; i < p.size(); i++) {
      int j = (i + 1) % p.size();
  ...
  double ComputeArea( std::vector<PT> &p) {
    return abs(ComputeSignedArea(p));
  }

  PT ComputeCentroid( std::vector<PT> &p) {
    PT c(0,0);
